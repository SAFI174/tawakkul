...
String getJuzNameQCF(int juzNumber) {
  return juz[juzNumber - 1]["qcf_name"];
}

String getJuzName(int juzNumber) {
  return juz[juzNumber - 1]["name"];
}
...
/// Generates a text representing the Hizb information based on the provided rubElHizbNumber
/// and hizbNumber.
///
/// [rubElHizbNumber] is the rubElHizbNumber associated with the page.
/// [hizbNumber] is the hizbNumber associated with the page.
///
/// Returns a formatted string containing the Hizb information.
String getHizbText({required int rubElHizbNumber, required int hizbNumber}) {
  ...
  return "$quarterDescription $hizb $hizbNumber";
}

String getSurahNameArabicSimple(int surahNumber) {
  if (surahNumber > 114 || surahNumber <= 0) {
    throw "No Surah found with given surahNumber";
  }
  return surah[surahNumber - 1]['arabic_simple'].toString();
}

String getSurahNameOnlyArabicSimple(int surahNumber) {
  if (surahNumber > 114 || surahNumber <= 0) {
    throw "No Surah found with given surahNumber";
  }
  return surah[surahNumber - 1]['arabic_simple']
      .toString()
      .replaceAll('سورة', '').trim();
}
...
Map<String, int> getHizbData(int hizbNumber) {
  if (hizbNumber > 60 || hizbNumber < 0) {
    throw "No Hizb found with given hizbNumber";
  }
  return hizbs[hizbNumber - 1];
}
...
int getVerseId(int surahNumber, int verseNumber) {
  for (int i = 0; i < quranText.length; i++) {
    if (quranText[i]["surah_number"] == surahNumber &&
        quranText[i]["verse_number"] == verseNumber) {
      return i + 1; // Add 1 to convert from 0-based index to 1-based index
    }
  }
  return 1; // Return null if the verse is not found
}
...
String getAudioURLByVerse(
    int surahNumber, int verseNumber, String reader, int audioRate) {
  int verseNum = 0;
  for (var i in quranText) {
    if (i['surah_number'] == surahNumber && i['verse_number'] == verseNumber) {
  ...
  return "https://cdn.islamic.network/quran/audio/$audioRate/$reader/$verseNum.mp3";
}
